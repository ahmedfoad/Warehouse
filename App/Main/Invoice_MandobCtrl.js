app.controller("Invoice_MandobCtrl", function ($scope, $http, $window, $timeout, $uibModal) {
    ...
    $scope.switchCase = function () {
        ...
        if (urlParams.has('id') == true) {
            ...
            $http.get('/Invoice_Mandob/loadInvoice/' + id)
    ...
    $scope.getView = function () {
        $http.get("/Invoice_Mandob/GetProducts")
         .success(function (response) {
    ...
    $scope.getAllMandob = function (Page, Search) {
        ...
        $http.get("/Invoice_Mandob/getAllMandob?" + sentData, { params: parameter })
       .success(function (response) {
    ...
    $scope.DeleteItem = function (invoice_product_id) {
        ...
        $http.post('/Invoice_Mandob/DeleteItem?' + sentData, parameter)
             .success(function (response) {
    ...
    //****************************************************************************
    var Customer_Typeview;
    $scope.AllCustomer_Type = function () {
        $http.get("/Invoice_Mandob/GetCustomer_Type")
            .success(function (response) {
                Customer_Typeview = response;
            });
    };
    $scope.AllCustomer_Type();
    $scope.loadCustomer_Type = function () {

        var CheckForm = $scope.Customer_TypeValidation();
        if (CheckForm == true) {
        $uibModal.open({
            template: Customer_Typeview,
            scope: $scope,
            resolve: {
                Invoice_Mandob: function () {
                    return $scope.Cls_Invoice_Mandob;
    ...
    $scope.Customer_TypeValidation = function () {
        $scope.AlertParameter = 0;
    ...
    $scope.getAllRecords_Customer_Type = function (Search) {
        var parameter = { 'Search': Search};
       
        $http.get("/Invoice_Mandob/getAllCustomer_Type", { params: parameter })
            .success(function (response) {
                if (response != null) {

                    if ($scope.Customer_TypeList == null) {
                        $scope.Customer_TypeList = response;
                    }
                    else {
                        $scope.Customer_TypeList = $scope.Customer_TypeList.concat(response);
                    }
                }
                //if (response == null || response == "") { otherData = 1; hideLoadMore = null; }
                //else {  otherData = null; hideLoadMore = 1; }
    ...
    $scope.getAllRecords_Customer_Type();
    $scope.clicktr_Customer_Type = function (row) {
        $scope.Cls_Invoice_Mandob.Customer_Type = row.Customer_Type;
        $scope.Cls_Invoice_Mandob.Customer_Name = row.Customer_Name;
        this.$close();
    }
    var lastsearch_Customer_Type = "";
    $scope.keyupevt_Customer_Type = function () {
        var search = $("#Search").val();
        if (search != "" && search != lastsearch_Customer_Type) {
            $scope.Customer_TypeList = null;
            page = 0;
            $scope.getAllRecords_Customer_Type(search);
        }
        lastsearch_Customer_Type = search;
    };
   
    //----------------------------------------------------------------------------------------------------------------------------
    var Payment_Typeview;
    $scope.AllPayment_Type = function () {
        $http.get("/Invoice_Mandob/GetPayment_Type")
            .success(function (response) {
                Payment_Typeview = response;
            });
    };
    $scope.AllPayment_Type();
    $scope.loadPayment_Type = function () {


        $uibModal.open({
            template: Payment_Typeview,
            scope: $scope,
            resolve: {
                Invoice_Mandob: function () {
                    return $scope.Cls_Invoice_Mandob;
    ...
    $scope.getAllRecords_Payment_Type = function (Search) {
        var parameter = { 'Search': Search };

        $http.get("/Invoice_Mandob/getAllPayment_Type", { params: parameter })
            .success(function (response) {
                if (response != null) {

                    if ($scope.Payment_TypeList == null) {
                        $scope.Payment_TypeList = response;
                    }
                    else {
                        $scope.Payment_TypeList = $scope.Payment_TypeList.concat(response);
                    }
                }
                //if (response == null || response == "") { otherData = 1; hideLoadMore = null; }
                //else {  otherData = null; hideLoadMore = 1; }
    ...
    $scope.getAllRecords_Payment_Type();
    $scope.clicktr_Payment_Type = function (row) {
        $scope.Cls_Invoice_Mandob.Payment_Type = row.Payment_Type;
        $scope.Cls_Invoice_Mandob.Payment_Type_Name = row.Payment_Type_Name;
        this.$close();


    }
    var lastsearch_Payment_Type = "";
    $scope.keyupevt_Payment_Type = function () {
        var search = $("#Search").val();
        if (search != "" && search != lastsearch_Payment_Type) {
            $scope.Payment_TypeList = null;
            page = 0;
            $scope.getAllRecords_Payment_Type(search);
        }
        lastsearch_Payment_Type = search;
    };
    //----------------------------------------------------------------------------------------------------------------------------
    ...
    $scope.BtnPrintInvoice = function () {
        ...
        if ($scope.Cls_Invoice_Mandob != null && $scope.Cls_Invoice_Mandob.ID != "" && $scope.Cls_Invoice_Mandob.ID != 0) {

            // $http.get("/Invoice_Mandob/PrintInvoiceDailog/" + $scope.Cls_Invoice_Mandob.ID)
            // .success(function (response) {
            //
            //View_Invoice = response;
            //$uibModal.open({
            //    scope: $scope,
            //    template: View_Invoice,
            //    resolve: {
            //        Cls_Invoice_Mandob: function () {
            //            return $scope.Cls_Invoice_Mandob;
            //        }
            ...
            $http({
                url: "/Invoice_Mandob/PdfInvoice?id=" + $scope.Cls_Invoice_Mandob.ID,
                method: "GET",
                headers: {
    ...
    $scope.Save = function (index) {
        ...
        if (CheckForm == true) {
            ...
            var parameter = JSON.stringify($scope.Cls_Invoice_Mandob);
            $http.post('/Invoice_Mandob/AddItem?' + sentData, parameter)
                 .success(function (response) {
    ...
    $scope.Delete = function () {
        ...
        $http.post('/Invoice_Mandob/DeleteInvoice?' + sentData)
             .success(function (response) {
    ...
    $scope.Edit = function (index, invoice_product_id) {
        ...
        if (CheckForm == true) {
            ...
            var parameter = JSON.stringify($scope.Cls_Invoice_Mandob);
            $http.post('/Invoice_Mandob/EditItem?' + sentData, parameter)
                 .success(function (response) {
                     if (response.ErrorName.indexOf("صلاحية") > -1 || response.ErrorName.indexOf("المسموح") > -1) {
                         $uibModal.open({
                             template: " <div class='portlet box red' style='min-height:150px !important ;min-width: 541px !important;'> <div class='portlet-title'> <div class='caption'> <i class='fa-lg fa fa-warning'></i> تنبيه هام </div><div class='tools'> </div></div><div class='portlet-body form portlet-empty' style='min-height: 140px;'> <div class='col-md-12 page-404'> <div class='number'>" + response.ErrorNumber + "</div><div class='details'> <h3 style='font-family: sans-serif; text-align: center; font-weight: bold;'>" + response.ErrorFullNumber + "</h3> <p> " + response.ErrorName + " <br/> <br/> <a class='btn btn-default btn-sm' href='/home'> الصفحة الرئيسية </a> </p></div></div></div></div>",
    ...
    $scope.EditMandob = function () {
        var CheckForm = $scope.FormValidation();
        if (CheckForm == true) {

            var parameter = JSON.stringify($scope.Cls_Invoice_Mandob);
            $http.post('/Invoice_Mandob/EditItemForMandob', parameter)
                 .success(function (response) {
                     if (response.ErrorName.indexOf("صلاحية") > -1 || response.ErrorName.indexOf("المسموح") > -1) {
                         $uibModal.open({
                             template: " <div class='portlet box red' style='min-height:150px !important ;min-width: 541px !important;'> <div class='portlet-title'> <div class='caption'> <i class='fa-lg fa fa-warning'></i> تنبيه هام </div><div class='tools'> </div></div><div class='portlet-body form portlet-empty' style='min-height: 140px;'> <div class='col-md-12 page-404'> <div class='number'>" + response.ErrorNumber + "</div><div class='details'> <h3 style='font-family: sans-serif; text-align: center; font-weight: bold;'>" + response.ErrorFullNumber + "</h3> <p> " + response.ErrorName + " <br/> <br/> <a class='btn btn-default btn-sm' href='/home'> الصفحة الرئيسية </a> </p></div></div></div></div>",
    ...
    $scope.getView = function () {
        $http.get("/Invoice_Mandob/AuthorizeManager")
         .success(function (response) {
    ...
    $scope.Remove = function () {
        ...
        var ID = $scope.Cls_Invoice_Mandob.ID;
        if (ID > 1) {
            $scope.AlertParameter = null;
            document.body.scrollTop = 0;
    ...
    $scope.AutorizeRemove = function () {
        ...
        $http.post('/Invoice_Mandob/IsAuthorizeManager', parameter)
                 .success(function (response) {
    ...
    $('#Date_Invoice').Zebra_DatePicker({
        onSelect: function (date) {
            ...
            if (CheckForm == true) {
                ...
                {
                    $http.post('/Invoice_Mandob/EditDateInvoice', parameter)
                         .success(function (response) {
                             var test = response;
                             if (response.ErrorName.indexOf("صلاحية") > -1 || response.ErrorName.indexOf("المسموح") > -1) {
                                 $uibModal.open({
        ...
        if (urlParams.has('id') == true) {
            ...
            $http.get('/Invoice_Mandob/loadInvoice/' + id)
               .success(function (response) {
                   ...
                   var CheckForm = $scope.Validation_Omola();
                   if (CheckForm == true) {
                       debugger;
                       if ($scope.Cls_Invoice_Mandob.Omola_Money == null || $scope.Cls_Invoice_Mandob.Omola_Money == "" || $scope.Cls_Invoice_Mandob.Omola_Money == 0) {
                       ...
                       $modelDialogOmola = $uibModal.open({
                           scope: $scope,
                           template: html_Omola,
                           resolve: {
                               Cls_Invoice_Mandob: function () {
    ...
    $scope.BtnSaveOmola = function () {
        ...
        var parameter = JSON.stringify($scope.Cls_Invoice_Mandob);
        $http.post('/Invoice_Mandob/SaveOmola', parameter)
             .success(function (response) {
    ...
    $scope.Omola_ConfirmView = function () {
        $http.get("/Invoice_Mandob/Omola_Confirm")
         .success(function (response) {
    ...
    $scope.PDF_SarfOmola = function () {
        $http({
            url: "/Invoice_Mandob/PDF_SarfOmola/" + $scope.Cls_Invoice_Mandob.ID,
            method: "GET",
            headers: {
                "Content-type": "application/pdf"
            },
    ...
    $scope.PrintMasrofat = function () {
        $http({
            url: "/Invoice_Mandob/Pdf_Masrofat/" + $scope.Cls_Invoice_Mandob.ID,
            method: "GET",
            headers: {
                "Content-type": "application/pdf"
            },
...
$("#Sadad_Invoice").click(function () {
    ...
    if (_id != "") {
        ...
        window.open("/Sadad_Mandob/SadadMandob?id=" + _id, '_blank');
    }
});