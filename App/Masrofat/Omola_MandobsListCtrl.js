var page = 0;
app.controller("OmolaMandobsListCtrl", function ($scope, $http, $window, $timeout, $uibModal) {
    $scope.Sadad_ID = null;
    $scope.Srch_OmolaMandobsList = { Sadad_Status: 1, Mandob_id: "", Mandob_Name: "" };
    //----------------------------------------------------------------------------------------------------------------
    $scope.emptyInputs = function () {
        $scope.Srch_OmolaMandobsList.Sadad_Status = 1;
        $scope.Srch_OmolaMandobsList.Mandob_id = "";
        $scope.Srch_OmolaMandobsList.Mandob_Name = "";
    };
    //----------------------------------------------------------------------------------------------------------------
    //----------------------------------------------------------------------------------------------------------------
    ...
    $scope.getAllRecords = function (Page) {
        ...
        var parameter = JSON.stringify({ 'RModel': $scope.Srch_OmolaMandobsList, 'page': page });
        $http.post("/OmolaMandob/Omola", parameter)
    ...
    $scope.loadMandobs = function () {
        ...
        $uibModal.open({
            ...
            resolve: {
                Srch_OmolaMandobsList: function () {
                    return $scope.Srch_OmolaMandobsList;
                }
            }
        });
    };
    $scope.clicktrMandob = function (row) {
        $scope.Srch_OmolaMandobsList.Mandob_id = row.ID;
        $scope.Srch_OmolaMandobsList.Mandob_Name = row.Name;
    ...
    $scope.OmolaMandob = function (item) {
      ...
      //  $scope.New_Sadad.Total_Sadad = item.Total_Sadad;
        ...
        $modelDialogSadad = $uibModal.open({
            scope: $scope,
            template: html_popup_OmolaMandob,
            resolve: {
                New_Sadad: function () {
    ...
    $scope.popup_OmolaMandob = function () {
        $http.get("/OmolaMandob/popup_OmolaMandob")
         .success(function (response) {
             html_popup_OmolaMandob = response;
         });


    };
    $scope.popup_OmolaMandob();
    //----------------------------------------------------------------------------------------------------------------------------
    ...
    $scope.BtnSaveOmola = function () {
        ...
        $http.post('/OmolaMandob/SaveList', parameter)
             .success(function (response) {
                 debugger;
    ...
    $scope.Omola_ConfirmView = function () {
        $http.get("/OmolaMandob/Omola_Confirm")
         .success(function (response) {
    ...
    $scope.PrintEradat = function () {
        $http({
            url: "/OmolaMandob/Pdf_Eradat/" + $scope.Sadad_ID,
            method: "GET",
            headers: {
